...
Public Class Frm_Sales
    ...
    Private Sub TXTFaccturePosTotal_EditValueChanged(sender As Object, e As EventArgs) Handles TXTFaccturePosTotal.EditValueChanged
        ...
        If Val(Replace(Me.TXTFaccturePosTotal.Text, ",", ".")) > 0 Then
            ...
            Me.TXT_Toitl.Text = Format(Val(Me.TXT_Toitl.Text.Trim), "000.000")
        End If
    ...
    Private Sub TXT_PoslnputCharges_KeyDown(sender As Object, e As KeyEventArgs)
        ' If String.IsNullOrEmpty(Me.TXTFaccturePosTotal.Text) OrElse String.IsNullOrEmpty(Me.TXT_PoslnputCharges.Text) Then Exit Sub
        'If Not IsNumeric(Me.TXTFaccturePosTotal.Text) OrElse Not IsNumeric(Me.TXT_PoslnputCharges.Text) Then Exit Sub
        Dim duplise As Double
        If e.KeyCode = Keys.Enter Then
            'duplise = Val(Me.TXT_PoslnputCharges.Text)
            'Me.TXT_PoslnputCharges.Text = vbNullString
            'Me.TXT_PoslnputCharges.Focus()
            'Me.txt_FacposCharges.Text = (Val(Me.TXT_PoslnputCharges.Text) + duplise).ToString("0.00")
            Beep()
        End If
    ...
    Private Sub TXT_PoslnputCharges_KeyPress(sender As Object, e As KeyPressEventArgs)
        'If Me.TXT_PoslnputCharges.Text.Trim = vbNullString Or Me.TXT_PoslnputCharges.Text.Trim = "00.00" Then
        '    Me.TXT_PoslnputCharges.Text = vbNullString

        'End If
        'If Char.IsControl(e.KeyChar) = False Then
        '    If Char.IsDigit(e.KeyChar) Or e.KeyChar = "." Then
        '        If Me.TXT_PoslnputCharges.Text.Contains(".") Then
        '            If Me.TXT_PoslnputCharges.Text.Split(".")(1).Length < 2 Then
    ...
    Private Sub GridView1_ValidateRow(sender As Object, e As ValidateRowEventArgs) Handles GridView1.ValidateRow
        ...
        If (Rassid_Stock < Rassid_Quantite) Then
            Sondoss_View.SetColumnError(Column_Items_Stock, "!! .. عذرا .. حقل رصيد المنتجات لا يجب أن يكون أصغر من رصيد كمية المنتجات")
            Sondoss_View.SetColumnError(Column_Items_Quantite, "!! .. عذرا .. حقل كمية المنتجات لا يجب أن يكون أكبر من رصيد مخزون المنتجات")
    ...
    Private Sub SimpleButton1_Click(sender As Object, e As EventArgs) Handles SimpleButton1.Click
        ...
        If Rresult = System.Windows.Forms.DialogResult.Yes Then
            ...
            Me.TxtFacturePosId.Text = RETRIEVE_MAXIMUM_RECORD_CODE_IN_TABLE("POS_MASTER_Id", "TBL_POS_MASTER")
            Me.TxtFacturePosCode.Text = Format(Val(RETRIEVE_MAXIMUM_RECORD_CODE_IN_TABLE("POS_MASTER_Id", "TBL_POS_MASTER") + 1), "POS000000")
            Me.TxtFacturePosDate.Text = DateTime.Now.ToString("yyyy-MM-dd")
            Me.TxtFacturePosHeure.Text = DateTime.Now.ToString("HH:mm:ss")
            Me.TxtFacturePosCustomers.Text = "CUS" & DateTime.Now.ToString("yyyyMMddHHmmss")
            clertext()
    ...
    Private Sub SimpleButton2_Click(sender As Object, e As EventArgs) Handles btnSeve.Click
        Try
            If GridView1.DataRowCount = 0 Then
                XtraMessageBox.Show("!! .. عذرا ..الرجاء اختيار منتج", "تنبيه", MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1)
                Exit Sub
            End If


            DB.Open()
            CMD = New SqlCommand("sql_add_TBLPOS_MASTER", DB.conn)
            CMD.CommandType = CommandType.StoredProcedure

            CMD.Parameters.AddWithValue("@POS_MASTER_Id", SqlDbType.Int).Value = Convert.ToInt32(Me.TxtFacturePosId.Text.Trim)
            CMD.Parameters.AddWithValue("@POS_MASTER_Code", SqlDbType.NVarChar).Value = Convert.ToString(Me.TxtFacturePosCode.Text.Trim)
            CMD.Parameters.AddWithValue("@POS_MASTER_Date", SqlDbType.Date).Value = Format(CDate(Me.TxtFacturePosDate.Text.Trim), "yyyy-MM-dd")
            CMD.Parameters.AddWithValue("@POS_MASTER_Heure", SqlDbType.NVarChar).Value = Convert.ToString(Me.TxtFacturePosHeure.Text.Trim)
            CMD.Parameters.AddWithValue("@POS_MASTER_Customers", SqlDbType.NVarChar).Value = Convert.ToString(Me.TxtFacturePosCustomers.Text.Trim)
            CMD.Parameters.AddWithValue("@POS_MASTER_Username", SqlDbType.NVarChar).Value = Convert.ToString(Me.TxtFacturePosUsername.Text.Trim)
            CMD.Parameters.AddWithValue("@POS_MASTER_Sous_Total", SqlDbType.Decimal).Value = Convert.ToDecimal(Me.TXTFaccturePosTotal.Text.Trim)
            CMD.Parameters.AddWithValue("@POS_MASTER_Remises", SqlDbType.Decimal).Value = Convert.ToDecimal(Me.txtFacturePosRemises.Text.Trim)
            CMD.Parameters.AddWithValue("@POS_MASTER_Grand_Total", SqlDbType.Decimal).Value = Convert.ToDecimal(Me.TXT_Toitl.Text.Trim)
            ...
            For I As Integer = 0 To Me.GridView1.DataRowCount - 1
                ...
                CMD = New SqlCommand("SQL_ADD_TBL_POS_DETAILS", DB.conn)
                CMD.CommandType = CommandType.StoredProcedure
            ...
            If Frm = DialogResult.Yes Then
                ...
                Dim fr As New frm_rp_workorder
                ...
                'Me.TxtFacturePosInputCharges.Text = "00.00"
                ' Me.TxtFacturePosQuantite.Text = "00"
                ' Me.TxtFacturePosSousTotal.Text = "00.00"
                ' Me.TxtFacturePosCharges.Text = "00.00"
                Me.txtFacturePosRemises.Text = "00.00"
                Me.TXTFaccturePosTotal.Text = "00.00"
                btnSeve.Enabled = False
                GridView1.OptionsBehavior.ReadOnly = True
    ...
    Sub logUser()
        ...
        da = New SqlDataAdapter("select ID, Name from Tbl_Customer", DB.conn)
        da.Fill(dt)
        If dt.Rows.Count > 0 Then
            ...
            TxtFacturePosCustomers.DataSource = Nothing
            Me.TxtFacturePosCustomers.DataSource = dt
            Me.TxtFacturePosCustomers.DisplayMember = "Name"
            Me.TxtFacturePosCustomers.ValueMember = "ID"

        End If
    End Sub

    Private Sub SimpleButton4_Click(sender As Object, e As EventArgs)
        frm_Sales_Returns.Show()
    End Sub
    Sub clertext()
        '  Me.TxtFacturePosId.Text = vbNullString
        ' Me.TxtFacturePosCode.Text = vbNullString
    ...
    Private Sub txtLiteral_EditValueChanged(sender As Object, e As EventArgs) Handles txtLiteral.EditValueChanged
